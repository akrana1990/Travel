...
*/	
	...
	require( WL_TEMPLATE_DIR_CORE . '/widgets/package-widget.php'); //Custom Recent Package Widget By Ankit
	require( WL_TEMPLATE_DIR_CORE . '/widgets/category-widget.php'); //Custom Category Widget By Ankit
	require( WL_TEMPLATE_DIR_CORE . '/widgets/extended-recent-post-widget.php');
	require( WL_TEMPLATE_DIR_CORE . '/cpt-custom-functions.php');
	require( WL_TEMPLATE_DIR_CORE . '/custom-meta-box.php');
	...
	function weblizar_scripts()
	{
		...
		wp_enqueue_style('open-sans', 'http://fonts.googleapis.com/css?family=Open+Sans:400,600,700');
		wp_enqueue_style('carousel', WL_TEMPLATE_DIR_URI . '/css/carousel.css');
        wp_enqueue_style('effects', WL_TEMPLATE_DIR_URI . '/css/effects.css');
        ...
        add_filter('script_loader_src', 'ank_jquery_local_fallback', 10, 2);
        wp_enqueue_script('flexslider', WL_TEMPLATE_DIR_URI .'/js/jquery.flexslider-min.js', array('jquery'),null,true);
		wp_enqueue_script('menu', WL_TEMPLATE_DIR_URI .'/js/menu/menu.js', array('jquery'),null,true);
		wp_enqueue_script('bootstrap-min-js', WL_TEMPLATE_DIR_URI .'/js/menu/bootstrap.min.js',null,null,true);
		wp_enqueue_script('docs-js', WL_TEMPLATE_DIR_URI .'/js/docs.min.js',null,null,true);
	}
...
function ank_jquery_local_fallback($src, $handle) {
    static $add_jquery_fallback = false;

    if ($add_jquery_fallback) {
        echo '<script>window.jQuery || document.write(\'<script src="' . get_template_directory_uri() . '/js/jquery.min.js">\x3C/script>\')</script>' . "\n";
...
// Read more tag to formatting in blog page
	...
	*/
	 if ( ( 'add_image_size' ) ) 
	 { 
		add_image_size('wl_media_sidebar_img',150,150,true);
		add_image_size('wl_media_blog_img',800,350,true);
		add_image_size('wl_blog_img',580,350,true);
	}
...
add_action('init', 'ank_remove_head_link');
...
add_filter('widget_posts_args', 'widget_posts_args_add_package');
function widget_posts_args_add_package($params) {
	$params['post_type'] = array('post', 'package');
	return $params;
}
...
<?php // set_post_thumbnail_size( 270, 213, true ); // 50 pixels wide by 50 pixels tall, crop mode ?>